...
dispatch_get('/', function() {
    ...
    $stmt = $db->prepare('SELECT memos.id, title, username, created_at FROM memos inner join users on memos.user = users.id WHERE is_private=0 ORDER BY created_at DESC, memos.id DESC LIMIT 100');
    $stmt->execute();
...
dispatch_get('/recent/:page', function(){
    ...
    $stmt = $db->prepare("SELECT memos.id, title, username, created_at FROM memos inner join users on memos.user = users.id WHERE is_private=0 ORDER BY created_at DESC, memos.id DESC LIMIT 100 OFFSET " . $page * 100);
    $stmt->execute();
    $memos = $stmt->fetchAll(PDO::FETCH_ASSOC);
